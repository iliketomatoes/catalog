...
def unicode_me(something):
    """Converts strings with non-ASCII characters to unicode for LOG. Python 3 doesn't have a `unicode()` function, so `unicode()` is an alias for `str()`, but `str()` doesn't take a second argument, hence this kludge."""  # noqa
    try:
...
def convert(obj, ids, attr_type, parent='root'):
    """Routes the elements of an object to the right function to convert them based on their data type"""  # noqa
...
def convert_dict(obj, ids, parent, attr_type):
    ...
    for key, val in obj.items():
        ...
        if isinstance(val, numbers.Number) or type(val) in (str, unicode):
            ...
            addline('<%s%s>%s</%s>' % (
                key, make_attrstring(attr), convert_dict(val, ids, key, attr_type), key)  # noqa
            )
        ...
        if isinstance(item, numbers.Number) or type(item) in (str, unicode):
            ...
            if not attr_type:
                addline('<item %s>%s</item>' % (make_attrstring(attr),
                                                convert_list(item, ids, 'item', attr_type)))
            else:
...
def dicttoxml(obj, root=True, custom_root='root', ids=False, attr_type=True):
    ...
    if root == True:
        ...
        addline('<%s>%s</%s>' % (custom_root,
                                 convert(obj, ids, attr_type, parent=custom_root), custom_root))
    else:
